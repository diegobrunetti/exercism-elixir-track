defmodule ScaleGenerator do
  ...
  def step(scale, tonic, step) do
    tonic_index = Enum.find_index(scale, fn e -> e == tonic end)
    apply_interval(scale, tonic_index, @steps[step])
  end

  defp apply_interval(scale, start, step) when start + step >= length(scale) do
    note_index = start + step - length(scale)
    Enum.at(scale, note_index)
  end

  defp apply_interval(scale, start, step) do
    Enum.at(scale, start + step, 0)
  end
  ...
  def chromatic_scale(tonic \\ "C") do
    do_chromatic_scale(String.upcase(tonic), [])
  end

  defp do_chromatic_scale(_tonic, acc) when length(acc) == 13 do
    Enum.reverse(acc)
  end

  defp do_chromatic_scale(tonic, acc) do
    next_note = step(@notes, tonic, @semitone)
    do_chromatic_scale(next_note, [tonic | acc])
  end
